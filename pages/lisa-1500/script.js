...
var title = 'LISA 1500 Patching Guide';
var cdr3 = new CDR1500;
          cdr3.config.frame={x:1900,y:0,scale:1}
          cdr3.config.rack={x:20,y:280,scale:0.535}
          cdr3.config.legend={x:60,y:55,scale:1}
          ...
          cdr3.config.legendVertical=false
...
function stopPreload(){
  ...
  //frame 3 - help page
    let help = s.rect(2860,10,930,730,5).attr({fill:'white'}),
        helpImg = openPNG(s,'https://dl.dropboxusercontent.com/s/cdamnx1vmnjtxox/CDR1500_about.png',2865,15,920,720,false);
...
function preLoadSVG(){
  preLoader();
  setTimeout(function(){
        CDR1500LISA = loadSVG(s,"https://dl.dropboxusercontent.com/s/x5yo446wnokkjew/CDR1500LISA.svg");
...
//CDR1500 object
function CDR1500(){
  const colors=['#005095','#E55A0D','#C1BA00','#6696bf','#005095','#E55A0D','#C1BA00','#6696bf','#005095','#E55A0D','#C1BA00','#6696bf','#005095','#E55A0D','#C1BA00','#6696bf','#005095','#E55A0D','#C1BA00','#6696bf'];
  ...
  this.config = {
    ...
    rack:{x:520,y:100,scale:0.73}, //position of rack (left top corner) and it's scale
    legend:{x:30,y:200,scale:1}, //position of legend (left top corner) and it's scale
...
let createFrame = function(){ 
    ...
    g6 = s.group().attr({id:'legendGr-'+obj.config.shapeName}) //legend    
    g = s.group(g1,g2,g3,g4,g5,g6).attr({id:obj.config.shapeName,transform:'matrix('+obj.config.frame.scale+',0,0,'+obj.config.frame.scale+','+obj.config.frame.x+','+obj.config.frame.y+')'}); //group everything
    g6.text(30,40,"Select first and second sequentially rack unit by clicking orange rectangles").attr({class:"Legend2",fontSize:'16px'});
...
function setChassis(num){
  switch(obj.PCstate.pressed){
    ...
    case 1:
      ...
      DeleteEl("#legend-"+obj.config.shapeName);      
      DeleteEl("#patchleads-"+obj.config.shapeName);
...
function addLegend(){
  ...
  let gr = g6.g()
                .attr({id:'legend-'+obj.config.shapeName})
      ...
      graphicStatic = function(){
        ...
        gr.rect(0,0,
                (obj.config.legendVertical)?(450):(800),//width
                (obj.config.legendVertical)?(330):(217)//height
                ,5)
          .attr({fill:'lightgrey',opacity:0.3,stroke:'grey'})
        //left and right rectangles
        gr.rect(10,10,90,60,5).attr({fill: colors[1],stroke:"white"})
        gr.text(20,50,
        ...
        gr.polygon(249.888,490,8.139,244.996,249.888,0,481.861,0,239.71,244.996,481.861,490).attr({
          transform:'matrix(-.1,0,0,.1,170,15)',fill:'grey'})
        gr.rect(190,10,90,60,5).attr({fill: colors[1],stroke:"white"})
        gr.text(200,50,
        ...
        //text
        gr.text(
          (obj.config.legendVertical)?(10):(350),
          (obj.config.legendVertical)?(110):(30),
          ((obj.config.useMinimalPCL)?('Shortest'):('Recommended'))+" routes"
        ).attr({class:"Legend1"})
      ...
      checkRecommended = function(){
        ...
        if(foundRecommended){
          gr.text(
            (obj.config.legendVertical)?(10):(350),
            (obj.config.legendVertical)?(140):(60),
            "with patch lead "+obj.PCstate.recPC+ ' m'
          ).attr({class:"Legend1"})
        } else {
          gr.text(
            (obj.config.legendVertical)?(10):(350),
      ...
      showRecommended = function(){
        let animatePath=function(event,strong,key){
          ...
          for(let key in obj.PCstate.recPCLNames){
            ...
            s.select('#animRect-'+obj.PCstate.recPCLNames[key]).attr({cursor:'default',stroke:'none',opacity:0.8}).unclick()
            s.select("#"+obj.PCstate.recPCLNames[key]).attr({opacity:0})
          }
          //animate path
             let  el = s.select("#"+animP),
                  elLength=el.getTotalLength();
                  s.select('#'+clRect).attr({stroke:'white',opacity:1})
                  ...
                  el.animate({strokeDashoffset : 0},1000, mina.linear,
                             ...
                             function(){
                               for(let key in obj.PCstate.recPCLNames){
                                 if (key == 7) {break}
                                  let currRect = s.select('#animRect-'+obj.PCstate.recPCLNames[key])
                                 if(currRect!==null){
        ...
        let xxx=(obj.config.legendVertical)?(10):(350),
            yyy=(obj.config.legendVertical)?(180):(80),
            cols=4;//yyy -  pixels from top, cols - how many cols
       
         for(let key in obj.PCstate.recPCLNames){
             let rectRec = gr.rect(xxx+110*key-(key-key % cols)*110,yyy+(key-key%cols)/cols*60,100,50,4).attr({id:'animRect-'+obj.PCstate.recPCLNames[key],fill:colors[key],opacity:0.8,cursor:'pointer',strokeWidth:'3px'}),
                 textRec = gr.text(0,
                                   0,
                            obj.PCstate.allPaths[obj.PCstate.recPCLNames[key]].code
                          ).attr({class:'Legend2',fontSize:'18px'})
             textRec.transform('matrix('+((textRec.getBBox().width>90)?(90/textRec.getBBox().width):(1))+',0,0,1,'+(xxx+5+110*key-(key-key % cols)*110)+','+(yyy+30+(key-key%cols)/cols*60)+')')
             rectRec.click(function(event){animatePath(event,false,key)})
           //after 8 variants stop showing
           if(key==7){
             gr.text(xxx,yyy+130,'there are more than 8 routes, only 8 are shown')
...
function homeFrame(){
  ...
  //other frames links
    ...
    recLen.click(function(){
      openPNG(s,'https://dl.dropboxusercontent.com/s/9ufdt1fumerqfv7/CDR1500_recommended_lengths.png',10,10,920,720,true)
    })
    ...
    recPatch.click(function(){
      openPNG(s,'https://dl.dropboxusercontent.com/s/hw77xez3pberfmy/CDR1500_recommended_vs_minimal.png',10,10,920,720,true)
    })
    //How to use the tool
    ...
    howTo.click(function(){
      openPNG(s,'https://dl.dropboxusercontent.com/s/3exgs3xzhl98f71/CDR1500_how_to_use.png',10,10,920,720,true)
    })
    ...
    let tgl_btn1 = new toggle_button_svg(
      function(){
        ...
        tgl_btn1.config.y=3
        tgl_btn1.config.scale=0.45
        tgl_btn1.config.color_b='#005095'
        tgl_btn1.config.color_f='#E55A0D'
        tgl_btn1.draw()
